class Commontator::Comment < ActiveRecord::Base
  ...
  has_one :translate_ja_comment, foreign_key: :commontator_comment_id, dependent: :destroy

  # after_create_commit :create_notification_with_comment
  # after_create_commit :translate_ja_attributes
  ...
  # add customized methods below

  def translate_ja_attributes
    attr = set_attributes
    return unless attr.any? { |_key, value| value.contains_japanese? }

    (translate_ja_comment || create_translate_ja_comment).update(attr)
    trans_attr = Commontator::Comment.google_trans(attr)
    update(trans_attr)
  end
  ...
  def translate_bg_job
    TranslateJaCommentJob.perform_async(id)
  end

  def create_notification_with_comment
    CommentNotification.with(
      heading: 'commented on ' + thread.commontable.heading, 
      profile_id: creator.profile.id
    ).deliver(thread.commontable.member)
  end
